...
def get_l1_l2_mean_score(gt_path, pred_path):
    ...
    gt_masked = np.where(mask, gt_image, 1)
    pred_masked = np.where(mask, pred_image, 1)
...
def np_l1_l2_metric_score(np_gt_dir, np_pred_dir):
    gt_path_list = list(Path(np_gt_dir).glob("*.npy"))
    pred_path_list = [os.path.join(np_pred_dir, gt_path.name) for gt_path in gt_path_list]
    l1_score_list, l2_score_list = [], []
...
def run(gt_depth_dir, gt_ply_dir, pred_depth_dir, pred_ply_dir, np_eval=False):
    ...
    if np_eval:
        l1_score, l2_score = np_l1_l2_metric_score(gt_depth_dir, pred_depth_dir)
    else:
